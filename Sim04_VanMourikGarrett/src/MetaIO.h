	...
	bool readFile(std::vector<std::string> &errlog)
	{
		...
		if ( metaFile.is_open() )
		{
			...
			while ( std::getline(lineStream, dataStr, ';') ) 
		    {
				...
				//****************************************************************************************
				/// Sudo code...
				//  1) create an empty array of vectors<MetaData>
				//     a) IF A{begin}, create a new vector
				//     b) UNTIL A{end}, add lineData to that vector
				//  2) create an empty queue of vectors<MetaData>
				//     a) FOR each vector in the above array add to the queue based on scheduling algorithm
				//          i) SFJ - add to queue based on size...min size first
				//         ii) FIFO - array.begin() to array.end()
				//        iii) PS - add to queue based on I/O count [add I/O counter to PCB block]
				//  3) print
				//     a) POP of each entry in the queue
				//     b) FOR each metadata entry in the queue vector
				//     		i) printLine()
